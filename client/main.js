...
function checkVotes(day) {
    ...
    if (day == "day") {
        if (player.votes > (totalPlayers.length / 2)) {
            ...
            if (totalMafia.length === 0) {
            ...
            } else { //if game is still runing -> turn gametime to night
                Games.update(game._id, {
                    $set: {
                        gameTime: "Night"
                    }
                ...
                var isInspectorStillAlive = Players.find({
                    'gameID': game._id,
                    'alive': true,
                    'role': 'inspector'
                }, {}).fetch();
                var isDoctorStillAlive = Players.find({
                    'gameID': game._id,
                    'alive': true,
                    'role': 'doctor'
                }, {}).fetch();
                ...
                if (game) {
                    ...
                    if (game.state == "waitingForPlayers") {
                        ...
                        Meteor.subscribe('players', game._id, function onReady() {
                            ...
                            var checkForSameName = Players.find({
                                'gameID': game._id,
                                'name': playerName
                            }).fetch();
...
Template.day.helpers({
    ...
    messageNotification: function() {
        var chat_messages = Chat.find({
            'game': Session.get("gameID"),
            createdAt: {
                $gt: Session.get("message_check")
            }
        }, {}).fetch();
    ...
    'click .vote': function(event) {
        ...
        if (game.state == "inspection") {
            if (myVote == player._id || player.alive == false || player.voteCast === myVote || votedPlayer.alive == false || votedPlayer.role == "narrator") {
                ...
                var isDoctorAlive = Players.find({
                    'gameID': game._id,
                    'alive': true,
                    'role': 'doctor'
                }, {}).fetch();
            ...
            if (player.alive == false || player.role == "narrator" || player.voteCast === myVote || votedPlayer.alive == false || votedPlayer.role == "narrator") {
                ...
                Players.update(Session.get('myVote'), {
                    $set: {
                        votes: lastVote.votes - 1
                    }
                });
                Session.set('myVote', myVote);
